...
Start with the Herdr skill absent from discovery but the Herdr binary available. Assert that the orchestrator runs `herdr --skill`, follows its version-matched guidance before issuing another Herdr command, and continues activation. If both discovery and that command fail, it stops instead of improvising CLI behavior.
