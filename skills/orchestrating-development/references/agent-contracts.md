...
Only a finalization handoff authorized by the task's responsible approval actor for the exact reviewed head permits the preparation skill, reviewer-owned context reconciliation, readiness change, and `pull-request-finalized`. Finalization never includes merge. A GitHub approval review requires a separate human request. Merge remains human-executed unless an explicit builder charter grants the orchestrator squash-merge authority for the exact non-main integration branch.
