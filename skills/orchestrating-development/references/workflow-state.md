...
Write `summary` as a short, specific status phrase describing current activity, blocker, or outcome—usually 3–5 words (e.g. “Awaiting registry reset choice”). Put the requested action in `next_action` and supporting context in result notes, not the table status. Use optional `next_role` for agent routing while working; omit it and `next_action` when complete. Human attention follows the state, not separate `attention_required`/`attention_reason` fields. Planning and reviewing are descriptions, not states; do not use `human-working` or infer human activity from an open workspace. Runtime idle/done alone is not completion.
...
The plugin observes starts/resumes and stops and queues `HERDR_AGENT_WAKE` notices. It coalesces pending updates to the latest observed state, keeps an in-flight notice distinct from later transitions, and defers while the coordinator is busy. Acknowledging a wake does not remove a persistent watch. Delivery failures have a bounded retry count; the durable inbox remains inspectable.
