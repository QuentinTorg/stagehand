...
Start managed Codex roles with the standard Stagehand sandbox and approval settings after installing the rule. Do not add a stricter sandbox or non-escalating approval override to implement publication safety: doing so can block ordinary builds, containers, signing, and other commands already governed by the user's approval policy. The task-worktree and user rules explicitly deny direct publication while allowing the guarded publisher, workflow events, Hunk, and normal approved development commands.
