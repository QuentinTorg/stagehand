...
Start typing to write in the message box, or focus it with a plain click in preview text or the box itself. Dragging preview text still selects it. The selected task and conversation keep refreshing while you write. Enter or Send submits and leaves the composer ready for a follow-up; Ctrl-J inserts a newline (Ctrl-G also sends). Esc or clicking another task keeps the draft. Arrow keys, Home/End, Backspace, and Delete edit text. The board adds only the workspace name and Herdr workspace ID (task ID only if no workspace exists), then sends your exact text to the verified bound controller pane. It never contacts a worker directly or treats delivery as workflow progress.
